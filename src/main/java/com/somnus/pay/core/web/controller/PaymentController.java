package com.somnus.pay.core.web.controller;
...
import com.somnus.pay.core.service.PayService;
import com.somnus.pay.core.service.PaymentService;
...
public abstract class PaymentController{
	...
	public String order(@Valid PaymentOrder paymentOrder, BindingResult result, HttpServletRequest request, Model model) {
		...
		try {
			...
			if (!isFromCache && result.hasErrors()) { // 如果合并成功,则代表使用新的支付模式,参数校验错误提示可以直接跳过
				...
				throw new PayException(msa.getMessage(MsgCodeList.ERROR_301015,new Object[]{error}));
			}
	...
	public String pay(PaymentOrder paymentOrder, BindingResult result, HttpServletRequest request, HttpServletResponse response, Model model) {
		...
		try {
			...
			PaymentOrder cachedPaymentOrder = /*(PaymentOrder) CacheServiceExcutor.get(iPayService.createPaymentStandbyKey(paymentOrder.getOrderId()))*/null;
			if (logger.isInfoEnabled()) {
